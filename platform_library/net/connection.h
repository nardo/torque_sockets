...
struct connection_parameters
{
   ...
   bool _using_crypto;                ///< Set to true if this connection is using crypto (public key and symmetric)
   ...
   ref_ptr<certificate> _certificate; ///< The certificate of the remote host.
   byte_buffer_ptr _shared_secret;      ///< The shared secret key 
   bool _request_key_exchange;         ///< The initiator of the connection wants a secure key exchange
   bool _request_certificate;         ///< The client is requesting a certificate
   uint8 _symmetric_key[symmetric_cipher::key_size]; ///< The symmetric key for the connection, generated by the client
   uint8 mInitVector[symmetric_cipher::key_size]; ///< The init vector, generated by the server
   ...
   bool _debug_object_sizes;           ///< This connection's initiator requested debugging size information during packet writes.
   ...
   connection_parameters()
   {
      ...
      _is_arranged = false;
#ifdef TORQUE_DEBUG
      _debug_object_sizes = true;
#else
      _debug_object_sizes = false;     
#endif
   }
};
...
protected:
	...
	virtual void on_connect_terminated(termination_reason reason, const char *rejection_string)
	{
	}
	
	/// Called when this established connection is terminated for any reason
	virtual void on_connection_terminated(termination_reason, const char *error_disconnect_string)
	{
	}
	...
	/// connection.
	virtual bool validate_certificate(certificate *the_certificate, bool is_initiator) { return true; }
	...
	virtual bool read_connect_request(bit_stream *stream, const char **error_string)
	{
		return true;
	}
	...
	virtual bool read_connect_accept(bit_stream *stream, const char **error_string)
	{
		stream;
		error_string;
		return true;
...
protected:
	...
	{
		if(_simulated_packet_loss && Random::readF() < _simulated_packet_loss)
		{
		...
		if(read_packet_header(bstream))
		{
			...
			if(!bstream->is_valid() && mErrorString.isEmpty())
				connection::setLastError("Invalid Packet.");
			if(!mErrorString.isEmpty())
				get_interface()->handleConnectionError(this, mErrorString);
			mErrorString = "";
		}
	}
	...
	void write_packet_header(bit_stream *bstream, net_packet_type packet_type)
	{
		if(window_full() && packet_type == data_packet)
			TORQUE_DEBUGBREAK();
		
		int32 ack_byte_count = ((_last_seq_recvd - _last_recv_ack_ack + 7) >> 3);
		Assert(ack_byte_count <= max_ack_byte_count, "ack_byte_count exceeds max_ack_byte_count!");
		...
		stream->writeRangedU32(ack_byte_count, 0, max_ack_byte_count);
		...
		if(send_delay > millisecondsToTime(2047))
			send_delay = millisecondsToTime(2047);
		stream->write_integer(uint32(send_delay.getMilliseconds() >> 3), 8);
	...
	bool read_packet_header(bit_stream *bstream)
	{
		...
		bool pk_data_packet_flag = pstream->readFlag();
		...
		Assert(pk_data_packet_flag, "Invalid packet header in connection::read_packet_header!");
		...
		uint32 pk_ack_byte_count   = pstream->readRangedU32(0, max_ack_byte_count);
		...
		for(uint32 i = 0; i < pk_ack_word_count; i++)
			pk_ack_mask[i] = pstream->read_integer(i == pk_ack_word_count - 1 ? 
												   (pk_ack_byte_count - (i * 4)) * 8 : 32);
		...
		time pk_send_delay = millisecondsToTime((pstream->read_integer(8) << 3) + 4);
		
		TorqueLogBlock(LogConnectionProtocol,
					   for(uint32 i = _last_seq_recvd+1; i < pk_sequence_number; i++)
					   logprintf ("Not recv %d", i);
					   logprintf("Recv %d %s", pk_sequence_number, packetTypeNames[pk_packet_type]);
					   );
	...
	void write_packet_rate_info(bit_stream *bstream, packet_notify *note)
	{
		...
		if(bstream->writeFlag(note->rate_changed))
		{
			if(!bstream->writeFlag(_type_flags.test(ConnectionAdaptive)))
			{
				bstream->writeRangedU32(_local_rate.max_recv_bandwidth, 0, max_fixed_bandwidth);
				bstream->writeRangedU32(_local_rate.max_send_bandwidth, 0, max_fixed_bandwidth);
				bstream->writeRangedU32(_local_rate.min_packet_recv_period, 1, max_fixed_send_period);
				bstream->writeRangedU32(_local_rate.min_packet_send_period, 1, max_fixed_send_period);
			}
		}
	}
	...
	void read_packet_rate_info(bit_stream *bstream)
	{
		...
		{
			if(bstream->readFlag())
				_type_flags.set(ConnectionRemoteAdaptive);
			else
			{
				_remote_rate.max_recv_bandwidth = bstream->readRangedU32(0, max_fixed_bandwidth);
				_remote_rate.max_send_bandwidth = bstream->readRangedU32(0, max_fixed_bandwidth);
				_remote_rate.min_packet_recv_period = bstream->readRangedU32(1, max_fixed_send_period);
				_remote_rate.min_packet_send_period = bstream->readRangedU32(1, max_fixed_send_period);
				compute_negotiated_rate();
			}
		}
	}
	...
	void keep_alive()
	{
		_last_ping_send_time = millisecondsToTime(0);
		_ping_send_count = 0;
	}
...
public:
	...
	void connect(interface *connection_interface, const address &address, bool request_key_exchange = false, bool request_certificate = false)
	{
		_connection_parameters._request_key_exchange = request_key_exchange;
		_connection_parameters._request_certificate = request_certificate;
	...
	bool connect_local(interface *connection_interface, interface *local_server_interface)
	{
		ref_object *co = ref_object::create(getClassName());
		connection *client = this;
		connection *server = dynamic_cast<connection *>(co);
		...
		client->set_interface(connection_interface);
		client->get_connection_parameters()._is_initiator = true;
		client->get_connection_parameters().mIsLocal = true;
		server->get_connection_parameters().mIsLocal = true;
		
		server->set_interface(serverInterface);
		
		server->set_initial_recv_sequence(client->get_initial_send_sequence());
		client->set_initial_recv_sequence(server->get_initial_send_sequence());
		client->set_remote_connection_object(server);
		server->set_remote_connection_object(client);
		
		stream.set_byte_position(0);
		client->write_connect_request(&stream);
		stream.set_byte_position(0);
		if(!server->read_connect_request(&stream, &error))
			goto errorOut;
		
		stream.set_byte_position(0);
		server->write_connect_accept(&stream);
		stream.set_byte_position(0);
		
		if(!client->read_connect_accept(&stream, &error))
			goto errorOut;
		
		client->set_connection_state(connection::connected);
		server->set_connection_state(connection::connected);
		
		client->on_connection_established();
		server->on_connection_established();
		connection_interface->addConnection(client);
	...
	void connect_arranged(interface *connection_interface, const array<address> &possible_addresses, nonce &myNonce, nonce &remote_nonce, byte_buffer_ptr shared_secret, bool is_initiator, bool requests_key_exchange = false, bool requests_certificate = false)
	{
		_connection_parameters._request_key_exchange = requests_key_exchange;
		_connection_parameters._request_certificate = requests_certificate;
		...
		_connection_parameters._nonce = nonce;
		_connection_parameters._server_nonce = server_nonce;
	...
	{
		_interface->disconnect(this, reason_self_disconnect, reason);
	}
...
private:
	BitMask32 _type_flags;  ///< Flags describing the type of connection this is, OR'd from NetConnectionTypeFlags.
	time _last_update_time;  ///< The last time a packet was sent from this instance.
	F32 _round_trip_time;   ///< Running average round trip time.
	...
	F32 _simulated_packet_loss; ///< Function to simulate packet loss on a network
	...
	void compute_negotiated_rate()
	{
		...
		if(_current_packet_send_size > udp_socket::MaxPacketDataSize)
			_current_packet_send_size = udp_socket::MaxPacketDataSize;
	}
...
public:
	...
	void set_remote_connection_object(connection *connection) { _remote_connection = connection; };
	connection *get_remote_connection_object() { return _remote_connection; }
...
public:
	...
	void setSymmetricCipher(symmetric_cipher *the_cipher)
	{
...
public:
	...
	udp_socket::Status send_packet(bit_stream *stream)
	{
		...
		if(is_local_connection())
		{
			...
			_remote_connection->read_raw_packet(stream);
			return udp_socket::OK;
		}
		else
		{
			...
			{
				_interface->sendto_delayed(get_address(), stream, _simulated_latency);
				return udp_socket::OK;
			}
			else
				return _interface->sendto(get_address(), stream);
		}
	}
	...
	bool check_timeout(time time)
	{
		if(!is_network_connection())
			return false;
		
		if(!_last_ping_send_time)
			_last_ping_send_time = time;
		
		time timeout = _ping_timeout;
		uint32 timeout_count = _ping_retry_count;

		if((time - _last_ping_send_time) > timeout)
		{
	...
	{
		time delay = millisecondsToTime( _current_packet_send_period );
		...
		if(!force)
		{
			...
			if(_send_delay_credit > millisecondsToTime(1000))
				_send_delay_credit = millisecondsToTime(1000);
		}
