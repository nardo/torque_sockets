...
struct ConnectionParameters
{
   ...
   uint32 mPuzzleDifficulty;            ///< Difficulty of the client puzzle solved by this client.
   uint32 mPuzzleSolution;              ///< Solution to the client puzzle the server sends to the client.
   uint32 mClientIdentity;              ///< The client identity as computed by the remote host.
   ref_ptr<asymmetric_key> mPublicKey; ///< The public key of the remote host.
   ref_ptr<asymmetric_key> mPrivateKey;///< The private key for this connection.  May be generated on the connection attempt.
   ref_ptr<certificate> mCertificate; ///< The certificate of the remote host.
   byte_buffer_ptr mSharedSecret;      ///< The shared secret key 
   ...
   uint8 mSymmetricKey[symmetric_cipher::KeySize]; ///< The symmetric key for the connection, generated by the client
   uint8 mInitVector[symmetric_cipher::KeySize]; ///< The init vector, generated by the server
   ...
   byte_buffer_ptr mArrangedSecret;    ///< The shared secret as arranged by the connection intermediary.
...
class NetConnection : public Object
{
   ...
   uint32 mLastSeqRecvdAtSend[MaxPacketWindowSize]; ///< The sequence number of the last packet received from the remote host when we sent the packet with sequence X & PacketWindowMask.
   uint32 mLastSeqRecvd;                            ///< The sequence number of the most recently received packet from the remote host.
   uint32 mHighestAckedSeq;                         ///< The highest sequence number the remote side has acknowledged.
   uint32 mLastSendSeq;                             ///< The sequence number of the last packet sent.
   uint32 mAckMask[MaxAckMaskSize];                 ///< long string of bits, each acking a packet sent by the remote host.
                                                 ///< The bit associated with mLastSeqRecvd is the low bit of the 0'th word of mAckMask.
   uint32 mLastRecvAckAck; ///< The highest sequence this side knows the other side has received an ACK or NACK for.

   uint32 mInitialSendSeq; ///< The first mLastSendSeq for this side of the connection.
   uint32 mInitialRecvSeq; ///< The first mLastSeqRecvd (the first mLastSendSeq for the remote host).
   ...
   uint32 mPingRetryCount; ///< Number of unacknowledged pings to send before timing out.
...
protected:
   ...
   virtual bool validatePublicKey(asymmetric_key *theKey, bool isInitiator) { return true; }

   /// Fills the connect request packet with additional custom data (from a subclass).
   virtual void writeConnectRequest(bit_stream *stream)
	{
	}
   ...
   virtual bool readConnectRequest(bit_stream *stream, const char **errorString)
	{
	   return true;
	}

   /// Writes any data needed to start the connection on the accept packet.
   virtual void writeConnectAccept(bit_stream *stream)
	{
	   stream;
	}

   /// Reads out the extra data read by writeConnectAccept and returns true if it is processed properly.
   virtual bool readConnectAccept(bit_stream *stream, const char **errorString)
	{
	   stream;
	   errorString;
	   return true;
   ...
   virtual void readPacket(bit_stream *bstream) {}
   ...
   virtual void writePacket(bit_stream *bstream, PacketNotify *note) {}
...
public:
   ...
   uint32 getNextSendSequence() { return mLastSendSeq + 1; }
   ...
   uint32 getLastSendSequence() { return mLastSendSeq; }

protected:
   /// Reads a raw packet from a bit_stream, as dispatched from NetInterface.
   void readRawPacket(bit_stream *bstream)
	{
	   ...
	   if(readPacketHeader(bstream))
	   {
		  ...
		  if(!bstream->is_valid() && mErrorString.isEmpty())
   ...
   void writeRawPacket(bit_stream *bstream, NetPacketType packetType)
	{
	   ...
	   if(packetType == DataPacket)
	   {
		  ...
		  int32 start = bstream->getBitPosition();
	   ...
	   if(!mSymmetricCipher.isNull())
	   {
		  ...
		  bit_stream_hash_and_encrypt(bstream, MessageSignatureBytes, PacketHeaderByteSize, mSymmetricCipher);
	   }
	}

   /// Writes the notify protocol's packet header into the bit_stream.
   void writePacketHeader(bit_stream *bstream, NetPacketType packetType)
	{
	   if(windowFull() && packetType == DataPacket)
		  TORQUE_DEBUGBREAK();

	   int32 ackByteCount = ((mLastSeqRecvd - mLastRecvAckAck + 7) >> 3);
	   ...
	   uint32 wordCount = (ackByteCount + 3) >> 2;
	   ...
	   stream->writeInt(uint32(sendDelay.getMilliseconds() >> 3), 8);
   ...
   bool readPacketHeader(bit_stream *bstream)
	{
	   ...
	   uint32 pkPacketType     = pstream->readInt(2);
	   uint32 pkSequenceNumber = pstream->readInt(5);
	   bool pkDataPacketFlg = pstream->readFlag();
	   pkSequenceNumber = pkSequenceNumber | (pstream->readInt(SequenceNumberBitSize - 5) << 5);

	   uint32 pkHighestAck     = pstream->readInt(AckSequenceNumberBitSize);
	   uint32 pkPadBits        = pstream->readInt(PacketHeaderPadBits);
	   ...
	   if(!mSymmetricCipher.isNull())
	   {
		  ...
		  if(!bit_stream_decrypt_and_check_hash(pstream, MessageSignatureBytes, PacketHeaderByteSize, mSymmetricCipher))
		  {
	   ...
	   uint32 pkAckByteCount   = pstream->readRangedU32(0, MaxAckByteCount);
	   if(pkAckByteCount > MaxAckByteCount || pkPacketType >= InvalidPacketType)
		  return false;
			 
	   uint32 pkAckMask[MaxAckMaskSize];
	   uint32 pkAckWordCount = (pkAckByteCount + 3) >> 2;

	   for(uint32 i = 0; i < pkAckWordCount; i++)
	   ...
	   TorqueLogBlock(LogConnectionProtocol,
		  for(uint32 i = mLastSeqRecvd+1; i < pkSequenceNumber; i++)
	   ...
	   uint32 upShifted = (pkPacketType == DataPacket) ? 1 : 0; 

	   for(uint32 i = 0; i < MaxAckMaskSize; i++)
	   {
		  uint32 nextShift = mAckMask[i] >> (32 - ackMaskShift);
		  mAckMask[i] = (mAckMask[i] << ackMaskShift) | upShifted;
		  upShifted = nextShift;
	   }

	   // do all the notifies...
	   uint32 notifyCount = pkHighestAck - mHighestAckedSeq;
	   for(uint32 i = 0; i < notifyCount; i++) 
	   {
		  uint32 notifyIndex = mHighestAckedSeq + i + 1;

		  uint32 ackMaskBit = (pkHighestAck - notifyIndex) & 0x1F;
		  uint32 ackMaskWord = (pkHighestAck - notifyIndex) >> 5;
	   ...
	   uint32 prevLastSequence = mLastSeqRecvd;
	...
	void writePacketRateInfo(bit_stream *bstream, PacketNotify *note)
	{
   ...
   void readPacketRateInfo(bit_stream *bstream)
	{
	   if(bstream->readFlag())
	   {
		  if(bstream->readFlag())
   ...
   void handleNotify(uint32 sequence, bool recvd)
	{
...
public:
   ...
   void setInitialRecvSequence(uint32 sequence)
	{ 
   ...
   uint32 getInitialRecvSequence() { return mInitialRecvSeq; }

   /// Returns the initial sequence number of packets sent to the remote host.
   uint32 getInitialSendSequence() { return mInitialSendSeq; }
   ...
   void connectArranged(NetInterface *connectionInterface, const Array<NetAddress> &possibleAddresses, nonce &myNonce, nonce &remoteNonce, byte_buffer_ptr sharedSecret, bool isInitiator, bool requestsKeyExchange = false, bool requestsCertificate = false)
	{
...
private:
   ...
   uint32 mSimulatedLatency;    ///< Amount of additional time this connection delays its packet sends to simulate latency in the connection
   ...
   struct NetRate
   {
      uint32 minPacketSendPeriod; ///< Minimum millisecond delay (maximum rate) between packet sends.
      uint32 minPacketRecvPeriod; ///< Minimum millisecond delay the remote host should allow between sends.
      uint32 maxSendBandwidth;    ///< Number of bytes per second we can send over the connection.
      uint32 maxRecvBandwidth;    ///< Number of bytes per second max that the remote instance should send.
   };
   ...
   void computeNegotiatedRate()
	{
	   ...
	   uint32 maxBandwidth = min(mLocalRate.maxSendBandwidth, mRemoteRate.maxRecvBandwidth);
	   mCurrentPacketSendSize = uint32(maxBandwidth * mCurrentPacketSendPeriod * 0.001f);
   ...
   uint32 mCurrentPacketSendSize;   ///< Current size of each packet sent to the remote host.
   uint32 mCurrentPacketSendPeriod; ///< Millisecond delay between sent packets.

   NetAddress mNetAddress;       ///< The network address of the host this instance is connected to.

   // timeout management stuff:
   uint32 mPingSendCount;    ///< Number of unacknowledged ping packets sent to the remote host
...
public:
   ...
   uint32 mConnectSendCount;    ///< Number of challenge or connect requests sent to the remote host.
...
protected:
	...
	ref_ptr<symmetric_cipher> mSymmetricCipher;
public:
	/// Sets the symmetric_cipher this NetConnection will use for encryption
	void setSymmetricCipher(symmetric_cipher *theCipher)
	{
...
public:
   ...
   void setPingTimeouts(Time timePerPing, uint32 pingRetryCount)
   ...
   void setSimulatedNetParams(F32 packetLoss, uint32 latency)
   ...
   /// Sends a packet that was written into a bit_stream to the remote host, or the mRemoteConnection on this host.
   NetSocket::Status sendPacket(bit_stream *stream)
	{
	   if(mSimulatedPacketLoss && Random::readF() < mSimulatedPacketLoss)
	   {
	   ...
	   TorqueLogMessageFormatted(LogNetConnection, ("NetConnection %s: SEND - %d bytes", mNetAddress.toString(), stream->get_byte_position()));
	   ...
	   if(isLocalConnection())
	   {
		  ...
		  uint32 size = stream->get_byte_position();
   ...
   bool checkTimeout(Time time)
	{
	   ...
	   uint32 timeoutCount = mPingRetryCount;
   ...
   void checkPacketSend(bool force, Time currentTime)
	{
	   ...
	   packet_stream stream(mCurrentPacketSendSize);
...
public:
   ...
   void setFixedRateParameters( uint32 minPacketSendPeriod, uint32 minPacketRecvPeriod, uint32 maxSendBandwidth, uint32 maxRecvBandwidth )
	{
...
private:
   uint32 mLastSeqRecvdAck;
   Time mLastAckTime;

   /// @}
};

static const uint32 MinimumPaddingBits = 128;       ///< Padding space that is required at the end of each packet for bit flag writes and such.
