...
void torque_socket_seed_entropy(torque_socket, uint8 entropy[32]);
///< Seed random entropy data for this socket (used in the generation of cryptographic keys).

void torque_socket_read_entropy(torque_socket, uint8 entropy[32]);
///< Read the current entropy state from this socket.

void torque_socket_set_private_key(torque_socket, uint32 key_data_size, uint8 *the_key);
...
void torque_connection_accept(torque_connection, uint32 connect_accept_data_size, uint8 connect_accept_data[torque_max_status_datagram_size]);
///< accept an incoming connection request.

void torque_connection_reject(torque_connection, uint32 reject_data_size, uint8 reject_data[torque_max_status_datagram_size]);
///< reject an incoming connection request.

void torque_connection_disconnect(torque_connection, uint32 disconnect_data_size, uint8 disconnect_data[torque_max_status_datagram_size]);
///< Close an open connection. 

send_to_result torque_connection_send_to(torque_connection, uint32 datagram_size, uint8 buffer[torque_max_datagram_size], uint32 *sequence_number);
